...
- PowerBI ingestion now emits `CorpUserInfoClass` instead of `CorpUserKeyClass` for user entities, providing richer user metadata (displayName, email, active status). A new config option `overwrite_existing_users` (default: `false`) prevents accidental overwrites of existing users managed by LDAP/SCIM/Okta. If you were relying on PowerBI to update user data, set `overwrite_existing_users: true` in your recipe. See [PowerBI docs](../../metadata-ingestion/docs/sources/powerbi/powerbi_pre.md#user-ownership-configuration) for details.
